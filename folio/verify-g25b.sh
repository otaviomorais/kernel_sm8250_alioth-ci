...
ok "folio_add_lru exportado" \
   "grep -q 'EXPORT_SYMBOL(folio_add_lru);' \"\$SW\"" "sem EXPORT_SYMBOL"
...
ok "filemap_add_folio exportado" \
   "grep -q 'EXPORT_SYMBOL_GPL(filemap_add_folio);' \"\$FL\"" "sem EXPORT_SYMBOL_GPL"
...
ok "o simbolo antigo nao e mais exportado" \
   "[ \"\$(nc 'EXPORT_SYMBOL_GPL(add_to_page_cache_lru)' \"\$FL\")\" = 0 ]" \
   "ainda exporta o nome antigo"

# --- os callers in-tree continuam compilando -------------------------
ok "os 3 callers de filemap.c seguem usando o wrapper" \
   "[ \"\$(nc 'add_to_page_cache_lru(' \"\$FL\")\" = 3 ]" "esperava 3"
